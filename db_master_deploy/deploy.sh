...
check_table() {
    ...
    if [[ -z $(psql -lqt -h localhost -U pgkogis -c "SELECT table_catalog||'.'||table_schema||'.'||table_name FROM information_schema.tables where lower(table_type) not like 'view'" -d ${source_db}  2> /dev/null | egrep "\b${source_id}\b") ]]; then
        echo "source table does not exist ${source_id} " >&2
        exit 1
    fi

    # check if target table exists 
    if [[ -z $(psql -lqt -h localhost -U pgkogis -c "SELECT table_catalog||'.'||table_schema||'.'||table_name FROM information_schema.tables where lower(table_type) not like 'view'" -d ${target_db}  2> /dev/null | egrep "\b${target_id}\b") ]]; then
    ...
    source_columns=$(psql -U pgkogis -h localhost -d ${source_db} -Atc "select column_name,data_type FROM information_schema.columns WHERE table_schema = '${source_schema}' AND table_name = '${source_table}' order by 1;")
    target_columns=$(psql -U pgkogis -h localhost -d ${target_db} -Atc "select column_name,data_type FROM information_schema.columns WHERE table_schema = '${target_schema}' AND table_name = '${target_table}' order by 1;")
...
check_database() {
    ...
    if [[ -z $(psql -lqt -h localhost -U pgkogis | egrep "\b${source_db}\b" 2> /dev/null) ]]; then
...
bod_create_archive() {
    ...
    if [[ ${source_db%_*} == bod ]]; then
        if [[ ${#timestamp} > 0 ]]; then
            ...
            psql -U pgkogis -d template1 -h localhost -c "COMMENT ON DATABASE ${archive_bod} IS 'snapshot/archive copy from ${source_db} on $(date '+%F %T') with command ${COMMAND} by user ${USER}';" > /dev/null
...
#######################################
copy_database() {
    size=$(psql -qAt -h localhost -U pgkogis -d ${source_db} -c "SELECT pg_size_pretty(pg_database_size('"${source_db}"'));")
    
    echo "copy ${source_db} to ${target_db} size: ${size} attached slaves: ${attached_slaves}"
    echo "creating temporary database ${target_db_tmp} ..."
    psql -U pgkogis -h localhost -d template1 -c "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname='${target_db_tmp}';" >/dev/null
    dropdb -U pgkogis -h localhost --if-exists ${target_db_tmp} &> /dev/null
    psql -U pgkogis -h localhost -d template1 -c "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname='${source_db}';" >/dev/null
    createdb -U pgkogis -h localhost -O postgres --encoding 'UTF-8' -T ${source_db} ${target_db_tmp} >/dev/null
...
copy_table() {
    ...
    AND    i.indisprimary;
    "
    primary_keys=$(psql -qAt -h localhost -U pgkogis -d ${source_db} -c "${primary_keys_sql}")

    jobs=${CPUS}
    rows=$(psql -qAt -h localhost -U pgkogis -d ${source_db} -c "SELECT count(1) FROM ${source_schema}.${source_table};")
    size=$(psql -qAt -h localhost -U pgkogis -d ${source_db} -c "SELECT pg_size_pretty(pg_total_relation_size('"${source_schema}.${source_table}"'));")
    ...
    psql -U pgkogis -h localhost --quiet --no-align  -t -c "
    SELECT conname,
    ...
    if [ "${#foreign_keys[@]}" -gt 0 ]; then
        ...
        do
            ...
            echo "ALTER TABLE IF EXISTS ONLY ${target_schema}.${target_table} DROP CONSTRAINT IF EXISTS ${i};" | psql -U pgkogis -h localhost -d ${target_db} &> /dev/null
        done
    fi

    echo "truncate table ${target_id}"
    ( psql -U pgkogis -h localhost -c "begin; TRUNCATE TABLE ${target_schema}.${target_table}; commit;" -d "${target_db}" )
    ...
    for ((i=1; i<=${jobs}; i++)); do
        ...
        ( psql -U pgkogis -h localhost -qAt -d ${source_db} -c "COPY ( SELECT * FROM ${source_schema}.${source_table} order by ${primary_keys:=1} asc offset ${offset} limit ${increment} ) TO STDOUT with csv" | psql -U pgkogis -h localhost -qAt -d ${target_db} -c "SET session_replication_role = replica; COPY ${target_schema}.${target_table} from stdin with csv; SET session_replication_role = DEFAULT;" )& pids="${pids} $!"
    done;  
...
# check db access
if [[ -z $(psql -lqt -h localhost -U pgkogis) ]]; then
